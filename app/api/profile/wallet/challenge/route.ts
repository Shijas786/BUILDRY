...
import { createWalletChallenge } from '@/lib/verifiedWalletServer'
...
export async function POST(req: NextRequest) {
  ...
  if (!chain || !address.trim()) {
    return NextResponse.json(
      { error: 'Send a valid chain ("sol" or "evm") and wallet address.' },
      { status: 400 }
    )
  }

  const result = await createWalletChallenge(adminDb, uid, chain, address)
  if (!result.ok) {
